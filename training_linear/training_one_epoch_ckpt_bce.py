...
def main_bce():
    ...
    excel_name = opt.backbone_training + '_' + opt.biomarker + opt.model + 'AUROC' + str(opt.patient_split) + '.csv'
...
def validate(val_loader, model, classifier, criterion, opt):
    ...
    r = roc_auc_score(label_array, out_array)
    return losses.avg, top1.avg,r


def validate_pat_level(val_loader, model, classifier, criterion, opt):
    """validation"""
    model.eval()
    classifier.eval()
    device = opt.device
