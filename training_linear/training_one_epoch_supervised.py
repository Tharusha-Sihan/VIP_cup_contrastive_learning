...
def main_supervised():
    ...
    for i in range(0, 3):
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            # train for one epoch
            time1 = time.time()
            loss, acc = train_supervised(train_loader, model, criterion,
                              optimizer, epoch, opt)
            time2 = time.time()
        ...
        loss, test_acc,prec, recall = validate_supervised(test_loader, model, criterion, opt)
...
def validate_supervised(val_loader, model,criterion, opt):
    """validation"""
    model.eval()
